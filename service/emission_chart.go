...
func BuildDelegationEmissionIncomeSeries(details []models.VestingDelegationEmissionDetail, chartRange *EmissionChartRange) ([]int64, []models.BigInt) {
	timestamps := make([]int64, 0, len(chartRange.WeekStarts))
	emissionAmounts := make([]models.BigInt, 0, len(chartRange.WeekStarts))
	bucketSums := make(map[int64]*big.Int, len(chartRange.WeekStarts))
	...
	for _, detail := range details {
		alignedStart, ok := AlignToMainnetEmissionWeekStart(detail.StartTime, chartRange.MainnetWeekStart)
		if !ok {
			continue
		}
		if alignedStart.Before(chartRange.RangeStart) || !alignedStart.Before(chartRange.RangeEnd) {
		...
		bucket.Add(bucket, &detail.EmissionAmount.Int)
	}
