...
- **Node name policy**. The relay MAY enforce node-name restrictions using the exact tuple (`GPUName`, `GPUVram`, `NodeVersion`), where `NodeVersion` is the node-reported `major.minor.patch`. When `task.node_name_whitelist_enabled` is true, only tuples in the node-name whitelist are eligible. Eligible tuples MUST also satisfy `task.minimum_node_name_number` using active-count data. Active counts include statuses `Available`, `Busy`, `PendingPause`, and `PendingQuit`, and exclude `Paused` and `Quit`. All GPU names MUST be stored and compared in normalized form: leading and trailing whitespace is removed, and every internal run of whitespace (spaces, tabs, newlines) is collapsed into a single space. The relay MUST apply this normalization at every input boundary that accepts a GPU name — node join (`gpu_name`), task creation (`required_gpu`), and the admin node-name whitelist APIs — so that whitelist and active-count lookups always operate on identical strings.
