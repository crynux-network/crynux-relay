...
func TestBuildEmissionTaskFeeCSVRowsAllocatesMicroCNXAndRemainder(t *testing.T) {
	...
	if rows[0].Address != "0xnode" || rows[0].Type != "node" || rows[0].TaskFee != "2.000000" || rows[0].Emission != "6.666666" || rows[0].StartTime != "1767830400" {
		t.Fatalf("unexpected node row: %+v", rows[0])
	}
	if rows[1].Address != "0xdelegator" || rows[1].Type != "delegation" || rows[1].TaskFee != "1.000000" || rows[1].Emission != "3.333333" || rows[1].StartTime != "1767830400" {
	...
	if rows[2].Type != "remainder" || rows[2].TaskFee != "0.000000" || rows[2].Emission != "0.000001" || rows[2].StartTime != "1767830400" {
